# CURRENT_STATE.md — ОПЕРАТИВНИЙ ЗРІЗ СТАНУ ПРОЄКТУ
> **ПРАВИЛО ПЕРШОЇ ДІЇ ДЛЯ БУДЬ-ЯКОЇ СЕСІЇ ТА МОДЕЛІ:**
> Прочитай цей файл першим. **ЗАБОРОНЕНО** починати сесію зі сліпого сканування директорій (`find_by_name`, `list_dir`, `grep_search`). Усі встановлені факти, координати файлів та відкриті задачі зафіксовані тут.

---

## 1. Головна мета та епістемічний протокол

1. **Ціль:** Створення автономної, непохитної бази знань та вузькопрофільного спеціаліста з калібрування дизельних систем (Pumpe-Düse / EDC16U34) на базі першоджерел (OEM-мануали, SSP, патенти, стандарти, A2L).
2. **Черговість робіт:** **СПОЧАТКУ знання та повний аналіз стоку, тюнінг — ПОТІМ.** Питання модифікації прошивки (Stage 1 / DPF OFF) розглядаються лише після повної готовності доказової бази.
3. **Захист від галюцинацій та підлабузництва:**
   - Жодне твердження не приймається без дослівної цитати або A2L-змінної (`python kb.py check`).
   - «Ой, ви праві» без нового першоджерела заборонено. Зміна позиції — це запис у БД з `retraction_reason`, а не речення в чаті.
   - Власні чернетки (`base/knowledge/drafts/`) мають статус `PROJECT_DRAFT` і не є доказом.

---

## 2. Паспорт автомобіля (ВСТАНОВЛЕНО — НЕ шукати й НЕ перепитувати)

| Параметр | Значення | Статус доказу |
|---|---|---|
| Авто | VW Golf 5 (1K1), 2008 | `ASSUMED` |
| Двигун | 1.9 TDI 8V Pumpe-Düse, **BLS**, 77 kW / 105 к.с. | `OEM_DOCUMENTED` |
| ECU | Bosch **EDC16U34**-3.42, процесор Motorola/Freescale MPC562 | `OEM_DOCUMENTED` |
| HW | `03G 906 021 QJ` | `MEASURED` (наліпка блоку) |
| SW | `1037391847`, проєкт `P447_HAXN` | `MEASURED` |
| Турбіна | **BorgWarner / KKK BV39A-0072** (OE: `03G253014M`, BW: `54399880072`) | ⚠ `INFERRED` (Garrett НЕ стосується цієї турбіни!) |
| КПП | 02M (6-ступенева механіка) | `ASSUMED` |

---

## 3. Точні шляхи до первинної істини (НЕ сканувати диски)

- **A2L-специфікація (16 772 об'єкти):**
  `D:\geminibase\golf5\diagnostic-review\definitions\03G906021QJ_1984_391847_P447_HAXN_EDC16U34_3.42\03G906021QJ_1984_391847_P447_HAXN_EDC16U34_3.42.a2l`
- **Еталон прошивки для аналізу:**
  `D:\geminibase\golf5\diagnostic-review\reference-from-hex.analysis-only.bin` (реконструйований з hex, sha256 `cf891152…`)
- **Декодовані значення карт наддуву стоку:**
  `D:\geminibase\golf5\diagnostic-review\math-engine\boost-path-stock-values.json`
- **Документація ланцюга наддуву стоку:**
  `D:\geminibase\golf5\docs\CONTROL-PATH-STOCK-boost.md`
- **База знань (SQLite + FTS5):**
  `D:\geminibase\golf5\ecu-kb\knowledge\kb.sqlite3`
- **Бібліотека літератури (78 першоджерел, 0 бінарників у git):**
  `D:\geminibase\base\knowledge\library\` та реєстр `base\knowledge\library\library_registry.json`
- **Робочі чернетки проєкту (21 файл):**
  `D:\geminibase\base\knowledge\drafts\`

---

## 4. Стан бази знань (`ecu-kb`)

- **Документів у базі:** 97 (76 `HAVE_LOCAL` + 21 `PROJECT_DRAFT`).
- **Текстових чанків:** 8 732 (усі прив'язані до точних номерів сторінок).
- **Ідентифікаторів:** 2 565 точних токенів (A2L, адреси `0x…`, парт-номери).
- **Тверджень (Claims):** 43 formal claims (наддув повністю покритий).
- **Цілісність (`python kb.py check`):** **0 порушень.**

### Швидкі команди роботи з базою знань:
```bat
cd D:\geminibase\golf5\ecu-kb
python kb.py status                 # загальний стан
python kb.py check                  # перевірка порушень цитувань
python kb.py claims                 # перегляд усіх затверджених тверджень
python kb.py search "<запит>"       # точний пошук за словом/номером
python kb.py a2l <префікс>          # перегляд карт A2L за групою (наприклад, PCR, FlMng)
python kb.py gaps                   # перелік відкритих прогалин
```

---

## 5. Поточний статус робіт і наступні кроки

1. **Завершено:**
   - Повний вертикальний зріз стоку наддуву: педаль $\to$ внутрішній момент $\to$ лімітери $\to$ паливо $\to$ уставка наддуву $\to$ PID $\to$ N75.
   - Очищення бібліотеки від чернеток і вилучення бінарних файлів з гіта.
   - Інгест першоджерел BorgWarner VTG, Turbo News BV39, MPC561/562 RM та ISO 14230-2.
2. **В роботі ЗАРАЗ:**
   - Побудова вертикального зрізу стоку по **паливоподачі та димовому обмеженню** (`CONTROL-PATH-STOCK-fuel.md`).
   - Аналіз тригерів післявпорскування (Post-Injection) та регенерації в OFF-прошивці для виявлення причини білого диму.
3. **Відкриті прогалини (Gaps P1):**
   - #1: Фото таблички турбіни на автомобілі.
   - #2: Заводський readback ECU з машини (поточний BIN реконструйовано з hex).
   - #3: Bosch Technical Instruction UIS/UPS (Yellow Jacket) — статус `WANT_USER_COPY`.
   - #12: Лог VCDS напрямку шпаруватості N75 проти фактичного наддуву.
