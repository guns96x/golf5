...
| 1 | `RESOLVED` | ~~Підтвердити BV39 фотом таблички~~ | фото зроблено й проаналізовано — `MEASURED (raw)`, деталі й застереження в `ecu-kb/claims/turbo-nameplate-photo.json` |
...
| 1 | `OPEN` | Дані BorgWarner за НОВИМИ ідентифікаторами `BV39A-0072` / `NE 1003/1756-00002` | компресорна карта / технічні межі; старий пошук за `54399880072` не мав сенсу — цього номера на табличці немає |
| 2 | `OPEN` | Прямо підтвердити рядок Kund-Nr на табличці — `03G` чи `036` | ця сесія фото не бачила, читання переказне; прямий погляд людини або чіткіший кадр |
...
| 2 | `RESOLVED` | ~~Просканувати `logs/` на замерзлу телеметрію~~ | зроблено (Gemini-інвентаризація 52 файлів + вибіркова ручна перевірка) |
| 2 | `RESOLVED` | ~~Виправити запис реєстру для «Pierburг»-документа~~ | не було потрібно — реєстр уже коректний, помилка була в моєму твердженні (`claim #22`, `deprecated`) |

**Безпечна межа наддуву для BV39 — усе одно UNKNOWN, попри фізичну
ідентифікацію.** Турбіна тепер `MEASURED` (BorgWarner BV39A-0072 /
NE 1003/1756-00002), але це не дає компресорної карти. Жоден інгестований
документ не містить допустимого тиску чи обертів саме для цих ідентифікаторів.
Значення мап `PCR_*` межею компонента не є. Фізична ідентифікація ≠ технічні
межі — це два різні claims (`ecu-kb/claims/turbo-nameplate-photo.json`), і
другий досі не закритий.
